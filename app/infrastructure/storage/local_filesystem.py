...
# 前端申请上传地址 -> 后端生成签名 URL -> 前端 PUT 文件 -> accept_signed_upload() 校验签名 -> 写入本地磁盘
class LocalFilesystemStorageProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self.public_base_url = public_base_url.rstrip("/")  # 去掉 URL 末尾的 /，避免拼接路径时产生 //
        self.signing_secret = signing_secret.encode("utf-8")  # HMAC 需要 bytes，因此把字符串密钥转换为 UTF-8 字节
    ...
    def _path(self, object_key: str) -> Path:  # 根据逻辑 object_key 计算文件在本地服务器上的真实路径
        if not object_key or object_key.startswith("/"):  # 禁止空 object_key，也禁止使用 / 开头的绝对路径
            raise AppError(
                "INVALID_OBJECT_KEY",
        ...
        candidate = (self.root / object_key).resolve()  # 把root object_key拼接，并解析..等路径成真正绝对路径

        if candidate == self.root or self.root not in candidate.parents:  # 确保最终路径必须位于 root 目录内部
    ...
    def _encode_key(object_key: str) -> str:  # 将object_key编码成适合放进URL路径的字符串
        ...
        ).decode().rstrip("=")  # 再转回字符串，并删除Base64尾部的=填充字
