...
# 存储端口及上传凭证模型，统一本地文件和对象存储的调用方式。  # 说明本文件的职责：定义存储抽象接口以及相关的数据模型
...
class UploadGrant(BaseModel):  # 定义“上传授权信息”模型，客户端获取该对象后即可按照其中的信息上传文件
    ...
    headers: dict[str, str] = Field(default_factory=dict)  # 上传时必须附带的 HTTP 请求头，默认创建一个新的空字典
...
class StorageProvider(Protocol):  # 定义统一的存储接口协议，具体实现可以是本地文件系统、S3、内存存储等
...
@runtime_checkable  # 允许 DirectTransferStorage 在运行时使用 isinstance(obj, DirectTransferStorage) 判断对象是否满足此协议
class DirectTransferStorage(Protocol):  # 定义直接HTTP文件传输能力，这是StorageProvider之外的一组可选能力
    ...
    async def accept_signed_upload(  # 接收通过签名URL上传过来的文件数据
        ...
        *,
        ...
        chunks: AsyncIterator[bytes],  # 异步字节流，每次读取一小块文件内容，避免一次性把整个文件加载到内存
    ...
    ) -> tuple[Path, ObjectMetadata]: ...  # 返回二元组：第一个是实际本地文件路径，第二个是文件对应的元数据